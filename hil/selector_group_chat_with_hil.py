...
def _build_user_proxy_agent(input_func=None):
    """Build UserProxyAgent for human-in-the-loop.

    Args:
        input_func: Optional callable(prompt: str) -> str.  When provided the
            agent reads human replies from this function instead of stdin.
            Pass a queue-backed function for UI integrations (e.g. Streamlit).
    """
    description = AGENT_DESCRIPTIONS.get("human_proxy_agent", "")
    kwargs = {"name": AGENT_NAMES["human_proxy_agent"], "description": description}
...
def build_team(input_func=None):
    """Build SelectorGroupChat with all participants and termination.

    Args:
        input_func: Optional callable(prompt: str) -> str passed to UserProxyAgent
            so that human input can be supplied by a UI instead of stdin.
    """
    return _build_team(input_func=input_func)


def _build_team(input_func=None):
    """Build SelectorGroupChat with all participants and termination."""
    participants = _build_assistant_agents() + [_build_user_proxy_agent(input_func=input_func)]
