"""
Tool implementations for MCP-based procurement workflow.

Copied from hil/tools.py; used by hil_mcp.server. All tools are:
- Deterministic
- Side-effect free
...
def create_purchase_request() -> Dict:
    """
    Create a purchase request (PR) after approval. Call this when the Reviewer
    recommended AUTO_APPROVE and the human has confirmed (e.g. approved, ok).
    Returns the new PR id and status.
    """
    return {
        "pr_id": generate_request_id(),
        "status": "CREATED",
    }
